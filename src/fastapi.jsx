...
export function FastapiPage({ dark, toggleTheme }) {
  return <PageFrame dark={dark} toggleTheme={toggleTheme} eyebrow="fastapi" title={<>Database Migrations for<br /><em>FastAPI + SQLAlchemy.</em></>} intro={<>Using Alembic with FastAPI? DBWarden is a declarative alternative for SQLAlchemy applications. The dbwarden-fastapi plugin provides sessions, health checks, and migrations from the same <code className="inline-code">dbwarden.py</code> file that generates your migrations.</>} install="dbwarden plugin add dbwarden-fastapi">
...
class Primary(DbwardenDatabase):
    ...
    database_url_sync = "postgresql://user:pass@localhost:5432/myapp"
    database_url_async = "postgresql+asyncpg://user:pass@localhost:5432/myapp"
    model_paths = ["app.models"]`}</CodeBlock>
      <p>Potential long-tail queries this page captures: <em>FastAPI PostgreSQL migrations</em>, <em>FastAPI SQLAlchemy PostgreSQL</em>, <em>FastAPI Alembic alternative</em>, <em>SQLAlchemy PostgreSQL migration tool</em>.</p>
    </div></section>
    <Faq items={[
