      ...
      <main id="top">
        ...
        <section className="section content-wrap plugin-section">
           <div className="section-label">/ 05 <span>extend the contract</span><a className="section-doc" href="/plugins">Browse all plugins ↗</a></div>
             <div className="plugin-layout"><div><h2>Core stays<br /><em>composable.</em></h2><p>Plugins add typed metadata, database objects, seeds, RBAC, FastAPI lifecycle helpers, and sandbox validation without turning the migration surface into a black box.</p><a className="text-link" href="https://github.com/dbwarden-org/dbwarden-plugin-template" target="_blank" rel="noreferrer">Create your own plugin <span>↗</span></a></div><div className="plugin-list">{plugins.map((plugin, index) => <div className={activePlugin === index ? 'plugin-item is-active' : 'plugin-item'} key={plugin.name}><button type="button" aria-expanded={activePlugin === index} onClick={() => setActivePlugin(index)}><span>0{index + 1}</span><strong>{plugin.name}</strong><span className="plugin-mark">{activePlugin === index ? '−' : '+'}</span></button>{activePlugin === index && <p>{plugin.detail}</p>}</div>)}</div></div>
        </section>
